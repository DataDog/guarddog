...
from tests.analyzer.metadata.resources.package_fixtures import pypi_package_info, npm_package_info
...
class TestNPMMetadataMismatch:
    ...
    def test_npm_metadata_check(self, mocker, npm_package_info, modification):
        package_json_metadata = deepcopy(npm_package_info["versions"][self.target_version])
        package_json_metadata = modified_version_info(modification[0], modification[1], package_json_metadata)
        npm_version_metadata = deepcopy(npm_package_info)
        npm_version_metadata = modified_npm_metadata_at_version(modification[0], modification[2], npm_version_metadata, self.target_version)
