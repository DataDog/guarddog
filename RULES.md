...
| install_scripts_appear | `threat.metadata.install-scripts-appear` | Identify a version that adds npm install-time lifecycle scripts (preinstall/install/postinstall) to a package whose earlier versions never used them. Install scripts appearing on an established script-free package are a common malicious-takeover vector, since npm runs them automatically on install. | medium | execution | | :white_check_mark: | | | | | |
