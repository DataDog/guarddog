...
class DeceptiveAuthorDetector(Detector):
    ...
    def _get_disposable_email_domains(self) -> list:
        ...
        disposable_email_domains_url = "https://raw.githubusercontent.com/wesbos/burner-email-providers/master/emails.txt"
        ...
        disposable_email_domains_path = os.path.join(resources_dir, disposable_email_domains_filename)

        disposable_email_domains_data = None

        if disposable_email_domains_filename in os.listdir(resources_dir):
            update_time = datetime.fromtimestamp(os.path.getmtime(disposable_email_domains_path))

            if datetime.now() - update_time <= timedelta(days=30):
                with open(disposable_email_domains_path, "r") as disposable_email_domains_file:
