...
class RooCodeParser(MCPConfigParser):
    client_name = "roo_code"

    def matches(self, path: str) -> bool:
        normalized = path.replace("\\", "/").lower()
        return normalized.endswith("mcp_settings.json") or normalized.endswith("/.roo/mcp.json")
        ...
        if isinstance(servers_obj, dict):
            for server_name, server_cfg in servers_obj.items():
                if not isinstance(server_cfg, dict):
                    continue
                scope = "project" if path.replace("\\", "/").lower().endswith("/.roo/mcp.json") else "user"
                servers.append(
                    self._make_server(
