...
class ClaudeCodeParser(MCPConfigParser):
    client_name = "claude_code"

    def matches(self, path: str) -> bool:
        normalized = path.replace("\\", "/").lower()
        return normalized.endswith("/.claude.json") or normalized.endswith("/.mcp.json")
    ...
    def parse(self, path: str) -> MCPConfigFile:
        ...
        scope = "project" if path.replace("\\", "/").lower().endswith("/.mcp.json") else "user"
